...
// Perfil com "Próximo Jogo do Meu Time" via API-Sports

import { createFileRoute, useNavigate, Link } from '@tanstack/react-router'
import { useEffect, useRef, useState } from 'react'
import { Camera, LogOut, TrendingUp, History, Headset, Loader2, Calendar, Trophy } from 'lucide-react'
...
function ProfilePage() {
  ...
  // Buscar jogos do time do coração
  ...
  useEffect(() => {
    ...
    if (user) {
      db
        .from('user_bets')
        .select('result_status,profit_loss')
        .eq('user_id', user.id)
        .then(({ data }: { data: { result_status: string; profit_loss: number }[] | null }) => {
          const arr = data ?? []
          setStats({
            total: arr.length,
  ...
  const hitRate = stats.total
    ? Math.round((stats.greens / Math.max(stats.greens + stats.reds, 1)) * 100)
    : 0
    ...
    <AppShell>
      <div className="space-y-5">
        ...
        <div className="rounded-2xl border border-arena-gray bg-arena-dark p-6 flex flex-col items-center text-center">
          <button onClick={() => fileRef.current?.click()} className="relative group">
            <Avatar className="w-24 h-24 border-2 border-arena-green">
              ...
              <AvatarFallback className="bg-arena-gray text-xl">
                {name.slice(0, 2).toUpperCase() || 'AR'}
              </AvatarFallback>
            </Avatar>
            <span className="absolute inset-0 rounded-full bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
          ...
          {profile?.role === 'admin' && (
            <span className="mt-2 px-2 py-0.5 rounded-full bg-arena-gold/20 text-arena-gold text-[10px] font-bold uppercase tracking-widest">
              Admin
            </span>
          )}
        </div>
          ...
          <div className="rounded-2xl border border-arena-gray bg-arena-dark p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-bold uppercase tracking-widest text-arena-gold">
                Próximo Jogo: {profile.favorite_team}
              </h3>
            </div>
            ...
            {!fixturesLoading && teamFixtures.length > 0 && (
              <div className="overflow-x-auto scrollbar-thin -mx-4 px-4">
                <div className="flex gap-3 w-max pb-2">
                  {teamFixtures.slice(0, 3).map((fixture) => (
            ...
            {!fixturesLoading && teamFixtures.length === 0 && (
              <div className="text-center py-4 text-arena-text-secondary">
                <Calendar className="w-8 h-8 mx-auto mb-2 opacity-40" />
                <p className="text-sm">Nenhum jogo encontrado para {profile.favorite_team} hoje.</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="rounded-2xl border border-arena-gray bg-arena-dark p-4 space-y-3">
          <h3 className="text-sm font-bold uppercase tracking-widest text-arena-text-secondary">
            Editar perfil
          </h3>
          <div className="space-y-2">
            ...
            <Input
              ...
              className="bg-arena-gray/40 border-arena-gray rounded-xl"
            />
          </div>
          ...
          <Button
            onClick={save}
            disabled={saving}
            className="w-full bg-arena-green text-black font-bold rounded-xl hover:bg-arena-green-dark"
          >
        ...
        <Link
          to="/history"
          className="flex items-center justify-between p-4 rounded-2xl border border-arena-gray bg-arena-dark hover:border-arena-green/50 transition"
        >
        ...
        <Button
          variant="outline"
          onClick={logout}
          className="w-full border-arena-red text-arena-red hover:bg-arena-red/10 rounded-xl"
        >
...
function Stat({
  ...
  return (
    <div className="rounded-2xl border border-arena-gray bg-arena-dark p-3 text-center">
      ...
      <p className="text-[10px] uppercase tracking-widest text-arena-text-secondary mt-1">
        {label}
      </p>
    </div>
  )
